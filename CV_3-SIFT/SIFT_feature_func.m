% SIFT_feature_func.m
% 
% Συνάρτηση που υλοποιεί τον αλγόριθμο SIFT για μια δοσμένη εικόνα.
% 
% Προσοχή: Βεβαιωθείτε ότι όλες οι απαραίτητες μεταβλητές επιστρέφονται σωστά.
%
% Προαπαιτούμενα:
% - Image Processing Toolbox
%
% Χρήση:
% [features, keypoints_x, keypoints_y] = SIFT_feature_func(image);
%
% Συγγραφέας: [Το Όνομά Σας]
% Ημερομηνία: [Ημερομηνία]

function [features, keypoints_x, keypoints_y] = SIFT_feature_func(img_input)
    %% Αρχικοποίηση
    tic
    row = 256;
    colum = 256;
    img = imresize(img_input, [row, colum]);
    img = im2double(img);
    origin = img;
    toc
    ...
    D = cell(1, octave);
    for i = 1:octave
        D{i} = zeros(row*2^(2-i)+2, colum*2^(2-i)+2, level);
    end
    ...
    for i = 1:extr_volume
        ...
        ry_p = y + 1;
        rz = extrema(4*(i-1)+2);
        z = D{extrema(4*(i-1)+1)}(rx_p, ry_p, rz);
        if (abs(z) < threshold)
            extrema(4*(i-1)+4) = 0;
        end
    end
    ...
    for i = 1:extr_volume
        ...
        ry_p = y + 1;
        rz = extrema(4*(i-1)+2);
        Dxx = D{extrema(4*(i-1)+1)}(rx_p-1, ry_p, rz) + D{extrema(4*(i-1)+1)}(rx_p+1, ry_p, rz) - 2*D{extrema(4*(i-1)+1)}(rx_p, ry_p, rz);
        Dyy = D{extrema(4*(i-1)+1)}(rx_p, ry_p-1, rz) + D{extrema(4*(i-1)+1)}(rx_p, ry_p+1, rz) - 2*D{extrema(4*(i-1)+1)}(rx_p, ry_p, rz);
        Dxy = D{extrema(4*(i-1)+1)}(rx_p-1, ry_p-1, rz) + D{extrema(4*(i-1)+1)}(rx_p+1, ry_p+1, rz) - D{extrema(4*(i-1)+1)}(rx_p-1, ry_p+1, rz) - D{extrema(4*(i-1)+1)}(rx_p+1, ry_p-1, rz);
        deter = Dxx * Dyy - Dxy^2;
        if (deter == 0)
            R = inf;
        else
    ...
    for i = 1:extr_volume
        ...
        x = floor((extrema(4*(i-1)+3)-1)/(n/(2^(extrema(4*(i-1)+1)-2)))) + 1;
        y = mod((extrema(4*(i-1)+3)-1), m/(2^(extrema(4*(i-1)+1)-2))) + 1;
        % Make sure the point is in the searchable area
        if(x > (width/2) && y > (width/2) && ...
           x < (m/2^(extrema(4*(i-1)+1)-2) - width/2 - 2) && ...
           y < (n/2^(extrema(4*(i-1)+1)-2) - width/2 - 2))
            rx_p = x + 1;
            ry_p = y + 1;
            rz = extrema(4*(i-1)+2);
            ...
            reg_mag = zeros(1, reg_volume);
            reg_theta = zeros(1, reg_volume);
            count = 1;
            for l = (rx_p - width/2):(rx_p + width/2 - 1)
                for k = (ry_p - width/2):(ry_p + width/2 - 1)
                    reg_mag(count) = sqrt((D{extrema(4*(i-1)+1)}(l+1, k, rz) - D{extrema(4*(i-1)+1)}(l-1, k, rz))^2 + ...
                                          (D{extrema(4*(i-1)+1)}(l, k+1, rz) - D{extrema(4*(i-1)+1)}(l, k-1, rz))^2);
                    reg_theta(count) = atan2((D{extrema(4*(i-1)+1)}(l, k+1, rz) - D{extrema(4*(i-1)+1)}(l, k-1, rz)), ...
                                            (D{extrema(4*(i-1)+1)}(l+1, k, rz) - D{extrema(4*(i-1)+1)}(l-1, k, rz))) * (180/pi);
                    count = count + 1;
                end
            end
            % Make histogram 
            ...
            for angle = 0:10:350
                ...
                mag_counts(angle/10 + 1) = mag_count;
            end
    ...
    % Επαναϋπολογισμός των συντεταγμένων μετά την ενημέρωση του 'extrema'
    x = floor((extrema(3:4:end)-1) ./ (n./(2.^(extrema(1:4:end)-2)))) + 1;
    y = mod((extrema(3:4:end)-1), m./(2.^(extrema(1:4:end)-2))) + 1;
    ry = y ./ 2.^(octave-1 - extrema(1:4:end));
    rx = x ./ 2.^(octave-1 - extrema(1:4:end));
    ...
    %% Keypoint Descriptor
    tic
    d = 4; % Divide the area into 4x4
    pixel = 4;
    features = zeros(d*d*8, extr_volume);
    for i = 1:extr_volume
        ...
        y = mod((extrema(4*(i-1)+3)-1), m / (2^(extrema(4*(i-1)+1)-2))) + 1;
        z = extrema(4*(i-1)+2);
        if ((m/2^(extrema(4*(i-1)+1)-2) - pixel*d*sqrt(2)/2) > x && x > (pixel*d/2 * sqrt(2)) && ...
            (n/2^(extrema(4*(i-1)+1)-2) - pixel*d/2 * sqrt(2)) > y && y > (pixel*d/2 * sqrt(2)))
            ...
            for p = 1:length(sub_x)
                for q = 1:length(sub_y)
                    sub(:, j_count) = [sub_x(p)-x; sub_y(q)-y];
                    j_count = j_count + 1;
                end
            end
            distort = [cos(pi*kpori(i)/180), -sin(pi*kpori(i)/180); ...
                       sin(pi*kpori(i)/180), cos(pi*kpori(i)/180)];
            % Coordinates after distortion
            sub_dis = distort * sub;
            ...
            for p = 1:length(fix_sub)
                ...
                if fix_sub(1,p) >=1 && fix_sub(1,p) <= size(D{extrema(4*(i-1)+1)},1) && ...
                   fix_sub(2,p) >=1 && fix_sub(2,p) <= size(D{extrema(4*(i-1)+1)},2)
                    patch(p) = D{extrema(4*(i-1)+1)}(fix_sub(1,p), fix_sub(2,p), z);
                else
                    patch(p) = 0; % Assign zero if out-of-bounds
                end
            end
            ...
            % Replace the descriptor section in SIFT_feature_func.m with this:
            for area=1:d*d
                cover=pixel*pixel;
                ori=zeros(1,cover);
                magcounts=zeros(1,8);
                for angle=0:45:359
                    magcount=0;
                    for p=1:cover
                        x_p=(floor((p-1)/pixel)+1)+pixel*floor((area-1)/d);
                        y_p=mod(p-1,pixel)+1+pixel*(mod(area-1,d));
                        c1=-180+angle;
                        c2=-180+45+angle;
                        if(c1<0||c2<0)
                            if (abs(theta_sub(x_p,y_p))<abs(c1)&&abs(theta_sub(x_p,y_p))>=abs(c2))
                                ori(p)=(c1+c2)/2;
                                magcount=magcount+mag_sub(x_p,y_p);
                            end
                        else
                            if(abs(theta_sub(x_p,y_p))>abs(c1)&&abs(theta_sub(x_p,y_p))<=abs(c2))
                                ori(p)=(c1+c2)/2;
    ...
    index = find(sum(features));
    features = features(:, index);
    % Extract keypoints' positions
    keypoints_x = rx(index);
    keypoints_y = ry(index);

    %% Ensure sizes match
    if (length(keypoints_x) ~= size(features, 2)) || (length(keypoints_y) ~= size(features, 2))
        error('Mismatch between number of features and keypoints_x/keypoints_y.');
    end

    %% Debugging: Display sizes
    disp(['Size of features after indexing: ', num2str(size(features,2))]);
    disp(['Size of keypoints_x: ', num2str(length(keypoints_x))]);
    disp(['Size of keypoints_y: ', num2str(length(keypoints_y))]);
